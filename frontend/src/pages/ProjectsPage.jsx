    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <Link to="/projects/add">
          <Button variant="primary"><i className="fa-solid fa-plus"></i> New Project</Button>
        </Link>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-black/5 dark:divide-white/5">
              {loading ? (
                ...
                return (
                  <tr key={proj.id} className="hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors cursor-pointer">
                    <td className="px-6 py-4">
                      <div className="font-bold text-slate-800 dark:text-slate-200">{proj.title}</div>
                      <div className="text-sm text-slate-500 dark:text-slate-400">Budget: ₹{Number(proj.budget || 0).toLocaleString()}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{proj.client_name || 'Unknown'}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm">
                      <div className={`font-semibold ${daysLeft < 0 ? "text-red-500" : daysLeft < 7 ? "text-amber-500" : "text-slate-600 dark:text-slate-400"}`}>
                        <i className="fa-regular fa-clock mr-1"></i>
                        {proj.status === "Completed" ? "Completed" : daysLeft < 0 ? `${Math.abs(daysLeft)} Days Overdue` : `${daysLeft} Days Left`}
                      </div>
                      {proj.deadline && <div className="text-xs text-slate-400 mt-0.5">Due: {new Date(proj.deadline).toLocaleDateString()}</div>}
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex gap-2">
                        <Link to={`/projects/edit/${proj.id}`} className="w-8 h-8 rounded-lg bg-black/5 dark:bg-white/5 text-slate-600 dark:text-slate-400 hover:bg-black/10 dark:hover:bg-white/10 flex items-center justify-center transition-colors">
                          <i className="fa-solid fa-pen"></i>
                        </Link>
                        <button onClick={() => deleteProject(proj.id)} className="w-8 h-8 rounded-lg bg-red-50 dark:bg-red-500/10 text-red-500 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-500/20 flex items-center justify-center transition-colors">
