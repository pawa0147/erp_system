...
const platformIcon: Record<string, { icon: string; color: string }> = {
  Google: { icon: "fa-google", color: "text-blue-500" },
  Facebook: { icon: "fa-facebook", color: "text-blue-700" },
  Instagram: { icon: "fa-pink-500", color: "text-pink-500" },
    ...
    <div className="space-y-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex gap-3">
          <Link href="/marketing/campaigns"><Button variant="secondary"><i className="fa-solid fa-list"></i> All Campaigns</Button></Link>
          <Link href="/marketing/create-campaign"><Button variant="primary" className="!bg-gradient-to-r !from-indigo-500 !to-purple-500"><i className="fa-solid fa-rocket"></i> Launch Campaign</Button></Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <GlassCard>
            <div className="flex justify-between items-center mb-4">
              ...
              <Link href="/marketing/campaigns" className="text-sm text-indigo-500 font-medium hover:underline">View All</Link>
            </div>
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5" suppressHydrationWarning>
                            <span className="text-emerald-500">• Active</span> | Conv: {cam.conversions} | Reach: {cam.reach.toLocaleString()}
                          </div>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="font-bold text-slate-800 dark:text-slate-200" suppressHydrationWarning>₹{cam.spend.toLocaleString()}</div>
                        <div className="text-xs text-slate-400 dark:text-slate-500" suppressHydrationWarning>of ₹{cam.budget.toLocaleString()}</div>
                        <Link href={`/marketing/${cam.id}/edit`} className="text-xs text-indigo-500 mt-1 inline-block"><i className="fa-solid fa-pen text-xs"></i> Edit</Link>
                      </div>
                    </div>
        ...
        <div>
          <GlassCard>
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2" suppressHydrationWarning><i className="fa-regular fa-calendar text-indigo-500"></i> {today.toLocaleDateString("en-IN", { month: "long", year: "numeric" })}</h3>
            </div>
                ...
                return (
                  <button key={day} onClick={() => setSelectedDay(day)}
                    className={`aspect-square rounded-lg text-xs font-medium transition-all flex items-center justify-center
                      ${isToday ? "bg-indigo-500 text-white font-bold shadow-md shadow-indigo-300" :
            ...
            {selectedDay && (
              <div className="mt-4 pt-4 border-t border-slate-100 dark:border-white/10">
                <p className="text-sm text-slate-500 dark:text-slate-400 italic" suppressHydrationWarning>No events for {today.toLocaleDateString("en-IN", { month: "long" })} {selectedDay}.</p>
              </div>
            )}
          </GlassCard>
        </div>
